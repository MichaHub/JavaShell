...
public class RawConsoleInput {
	...
	private static void shutdownHook() {
		try {
			...
			System.out.println("shutdownHook failure" + e.getMessage());
		}
	}
...
// The Windows version uses _kbhit() and _getwch() from msvcrt.dll.
	...
	private static int readWindows(boolean wait) throws IOException {
		...
		int newRawMode = (originalConsoleMode & ~Kernel32Defs.ENABLE_PROCESSED_INPUT & ~Kernel32Defs.ENABLE_LINE_INPUT
				& ~Kernel32Defs.ENABLE_ECHO_INPUT) | Kernel32Defs.ENABLE_VIRTUAL_TERMINAL_INPUT;

		consoleModeAltered = true;
		setConsoleMode(consoleHandle, newRawMode);
	...
	private static synchronized void initWindows() throws IOException {
		...
		try {
			Pointer outputHandle = kernel32.GetStdHandle(Kernel32Defs.STD_OUTPUT_HANDLE);
			int outputMode = getConsoleMode(outputHandle);
			setConsoleMode(outputHandle, outputMode | Kernel32Defs.ENABLE_VIRTUAL_TERMINAL_PROCESSING);
		} catch (IOException e) {
			System.out.println("wrong output" + e.getMessage());
		}
